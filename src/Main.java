...
демонстрирую в тестах в классе Main:
    3.1. Epic, getSubtaskIds(): "геттер должен возвращать существующее поле - subtaskIds, иначе его смысл теряется.".
    ...
    3.2. Task, Epic, Subtask, Manager: "использование и переопределение clone() излишне. <...> Не нужен по ТЗ."
    ...
    - rewrite(Epic).
        ...
        то ситуация аналогична 3.1: заложим баги с возможностью изменять поля без сеттеров. Вроде как без надобности
        (Возможно я всего не знаю - например, программа будет развиваться, и клоны начнут мешать. Значит так тому
        быть?:) Тогда это решения для следующих этапов.).
        Тесты, в которых воссоздал баги, добавлены в Main:
